...
const DB_PERSON = [
  {
    ...
    password: "123456", // Mật khẩu để login
...
function handleLogin() {
  ...
  // Tìm user thỏa mãn TẤT CẢ 5 điều kiện:
  // 1. Mã số khám khớp
  // 2. Ngày sinh khớp
  // 3. Email khớp
  // 4. Password khớp
  // 5. Chưa bị xóa (deleted_flag = 0)
  const user = DB_PERSON.find(
    (p) =>
      ...
      p.password == pass && // So sánh password
  ...
  document.getElementById("msg_E0000").innerHTML = MESSAGES.E0000_SUCCESS; // Nội dung từ messages.js
...
function clearForm() {
  ...
  // --- RESET TẤT CẢ DROPDOWN (SELECT) VỀ OPTION ĐẦU TIÊN ---
  // Reset các dropdown năm/tháng/ngày về lựa chọn đầu tiên
